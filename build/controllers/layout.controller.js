...
exports.createLayout = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        ...
        if (isTypeExist) {
            return next(new ErrorHandler_1.default(`${type} Layout already exist.`, 400));
        }
        if (type === "Banner") {
...
exports.editLayout = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        ...
        if (type === "FAQ") {
            ...
            if (!faqItem) {
                return next(new ErrorHandler_1.default(`FAQ Item does not exist.`, 400));
            }
            const faqItems = await Promise.all(faq.map(async (item) => {
                return {
                    question: item.question,
        ...
        if (type === "Categories") {
            ...
            if (!category) {
                return next(new ErrorHandler_1.default(`Categories Item does not exist.`, 400));
            }
            const categoriesItems = await Promise.all(categories.map(async (item) => {
                return {
                    title: item.title,
...
exports.getLayoutByType = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        ...
        if (!layout) {
            return next(new ErrorHandler_1.default(`Layout does not exist.`, 400));
        }
        res.status(200).json({
            success: true,
            layout,
